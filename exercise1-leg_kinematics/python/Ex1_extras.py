...
class TrajectoryPlotter:
    ...
    def r_B2_inB(self, q):
        return np.array([np.zeros_like(q[0]), np.sin(q[0]) + 1, -np.cos(q[0])])
    ...
    def plot(self):
        ...
        (pL1,) = self.h_ax[0].plot(
            [self.rKArr[0, 0], self.rArr[0, 0]],
            [self.rKArr[2, 0], self.rArr[2, 0]],
            "k-.",
        )
        (pL2,) = self.h_ax[0].plot(
            [self.rHArr[0, 0], self.rKArr[0, 0]],
            [self.rHArr[2, 0], self.rKArr[2, 0]],
            "k-.",
        )
        (pL3,) = self.h_ax[0].plot([0, self.rHArr[0, 0]], [0, self.rHArr[2, 0]], "k-.")
        self.h_ax[0].legend(
            [pG, pA, pL1], ["Target position", "Actual position", "Leg configuration"]
        )
    ...
    def _animate(self, i):
        ...
        self.artists[2].set_data(
            [self.rKArr[0, i], self.rArr[0, i]], [self.rKArr[2, i], self.rArr[2, i]]
        )
        self.artists[3].set_data(
            [self.rHArr[0, i], self.rKArr[0, i]], [self.rHArr[2, i], self.rKArr[2, i]]
        )
        self.artists[4].set_data([0, self.rHArr[0, i]], [0, self.rHArr[2, i]])
